...
| 单指**直接落在触摸板下沿**并左右滑动 | 屏幕下方浮出切换指示条，按窗口层叠顺序逐个把窗口带到前台预览；滑得越快切得越快（最高 4 倍加速），**抬起手指即选定**当前窗口，指针移到窗口中心并脉冲放大一下提示落点。从别处滑进下沿不会触发 |
...
- **设置窗口**：顶部触摸板示意图实时显示各功能区和你当前的手指位置，**直接拖动示意图上的白色手柄调整各热区大小**；每个手势可单独开关，窗口切换的方向与指针跟随可配置
...
- `switcherEnabled`（默认 true）/ `bottomEdgeHeight`（0.09）/ `switcherStepDistance`（0.055，慢滑一格的距离，快滑最高 4 倍加速）/ `switcherRightToNext`（true，向右滑切换下一个窗口）/ `switcherMovesPointer`（true，选定后把指针移到所选窗口中心）：下沿滑动切换窗口
